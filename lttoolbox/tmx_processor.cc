 ...
 * 02111-1307, USA.
 */
#include <lttoolbox/tmx_processor.h>
...
wchar_t
TMXProcessor::readEscaped(FILE *input)
{
  if(feof(input))
  {
    streamError();
...
wstring 
TMXProcessor::readFullBlock(FILE *input, wchar_t const delim1, wchar_t const delim2)
{
  wstring result = L"";
  result += delim1;
  wchar_t c = delim1;
...
int
TMXProcessor::readAnalysis(FILE *input)
{
...
int
TMXProcessor::readPostgeneration(FILE *input)
{
  if(!input_buffer.isEmpty())
  {
    return input_buffer.next();
...
void
TMXProcessor::skipUntil(FILE *input, FILE *output, wint_t const character)
{
  while(true)
  {
...
int
TMXProcessor::readGeneration(FILE *input, FILE *output)
{
  wint_t val = fgetwc_unlocked(input);
...
void
TMXProcessor::flushBlanks(FILE *output)
{  
...
bool
TMXProcessor::endsWith(wstring const &str, wstring const &suffix)
{
...
void
TMXProcessor::writeEscaped(wstring const &str, FILE *output)
{
...
void
TMXProcessor::printWord(wstring const &sf, wstring const &lf, FILE *output)
{
  fputwc_unlocked(L'^', output);
...
void
TMXProcessor::printUnknownWord(wstring const &sf, FILE *output)
{
  fputwc_unlocked(L'^', output);
  writeEscaped(sf, output);
  fputwc_unlocked(L'/', output);
...
unsigned int
TMXProcessor::lastBlank(wstring const &str)
{
...
void
TMXProcessor::printSpace(wchar_t const val, FILE *output)
{
...
bool
TMXProcessor::isEscaped(wchar_t const c) const
{
  return escaped_chars.find(c) != escaped_chars.end();
}

bool
TMXProcessor::isAlphabetic(wchar_t const c) const
{
  return alphabetic_chars.find(c) != alphabetic_chars.end();
}

void
TMXProcessor::load(FILE *input)
{
  // letters
...
void
TMXProcessor::initPostgeneration()
{
  initGeneration();
}

void
TMXProcessor::initBiltrans()
{
  initGeneration();
}

void
TMXProcessor::analysis(FILE *input, FILE *output)
{
...
void
TMXProcessor::generation(FILE *input, FILE *output, GenerationMode mode)
{
  State current_state = initial_state;
  wstring sf = L"";
...
void
TMXProcessor::postgeneration(FILE *input, FILE *output)
{
  bool skip_mode = true;
  State current_state = initial_state;
  wstring lf = L"";
...
wstring
TMXProcessor::biltrans(wstring const &input_word, bool with_delim)
{
  State current_state = initial_state;
  wstring result = L"";
...
pair<wstring, int>
TMXProcessor::biltransWithQueue(wstring const &input_word, bool with_delim)
{
  State current_state = initial_state;
  wstring result = L"";
  unsigned int start_point = 1;
...
wstring
TMXProcessor::biltransWithoutQueue(wstring const &input_word, bool with_delim)
{
  State current_state = initial_state;
  wstring result = L"";
  unsigned int start_point = 1;
...
bool
TMXProcessor::valid() const
{
  return !initial_state.isFinal(all_finals);
}

int
TMXProcessor::readSAO(FILE *input)
{
  if(!input_buffer.isEmpty())
  {
    return input_buffer.next();
...
void
TMXProcessor::printSAOWord(wstring const &lf, FILE *output)
{
...
void
TMXProcessor::SAO(FILE *input, FILE *output)
{
  bool last_incond = false;
  bool last_postblank = false;
  State current_state = initial_state;
...
wstring
TMXProcessor::removeTags(wstring const &str)
{
