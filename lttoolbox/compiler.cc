                           ...
                           int state, Transducer &t)
{
  ...
  {
    state = t.insertNewSingleTransduction(alphabet(0, 0), state);
  }
  else
  {
    ...
    while(true)
    {
      ...
      int new_state = t.insertSingleTransduction(tag, state);
      ...
      if(acx_map_ptr != acx_map.end())
      {
        ...
        { 
          t.linkStates(state, new_state, alphabet(*it ,rsymbol));
        }
      }
      state = new_state;
    }
